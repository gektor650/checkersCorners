 ...
 * */
 ...
 public class Graph {
    ...
    public Graph( List<MoveLine> lines ) {
        ...
        //Определяем для каждой связи номер поля на шахматной доске ( считаем что на доске 64 поля(ChessGame.CHESSBOARD_FIELDS_COUNT*ChessGame.CHESSBOARD_FIELDS_COUNT))
        int[] iToFieldNumberBeg = new int[ lines.size() ];
        int[] iToFieldNumberEnd = new int[ lines.size() ];
        for( i = 0 ; i < lines.size() ; i++ ) {
            iToFieldNumberBeg[i] = lines.get(i).getPosition1().getY() * ChessGame.CHESSBOARD_FIELDS_COUNT + lines.get(i).getPosition1().getX();
            iToFieldNumberEnd[i] = lines.get(i).getPosition2().getY() * ChessGame.CHESSBOARD_FIELDS_COUNT + lines.get(i).getPosition2().getX();
        ...
        for( i = 0 ; i < lines.size() ; i++ ) {
            ...
            tmp1 = lines.get(i).getPosition1().getY() * ChessGame.CHESSBOARD_FIELDS_COUNT + lines.get(i).getPosition1().getX();
            tmp2 = lines.get(i).getPosition2().getY() * ChessGame.CHESSBOARD_FIELDS_COUNT + lines.get(i).getPosition2().getX();
            for( j = 0 ; j < lines.size() ; j++ ) {
                ...
                vertex2 = -1;
                //Переводим координаты поля в номер фигуры (0 - 63 для стандартной шахматной доски)
                tmp3 = lines.get(j).getPosition1().getY() * ChessGame.CHESSBOARD_FIELDS_COUNT + lines.get(j).getPosition1().getX();
                tmp4 = lines.get(j).getPosition2().getY() * ChessGame.CHESSBOARD_FIELDS_COUNT + lines.get(j).getPosition2().getX();
//               Ищем пересечения линий
    ...
    public List<MoveLine> getShortestRoute( int x1 , int y1 , int x2 , int y2  ) {
        ...
        //Переводим координаты поля в номер фигуры (0 - 63 для стандартной шахматной доски)
        int position1 = y1 * ChessGame.CHESSBOARD_FIELDS_COUNT + x1;
        int position2 = y2 * ChessGame.CHESSBOARD_FIELDS_COUNT + x2;
        ...
        for( i = 1 ; i < path.size() ; i++ ) {
            lines.add( new MoveLine(
                    path.get(i-1) % ChessGame.CHESSBOARD_FIELDS_COUNT ,
                    (int) path.get(i-1) / ChessGame.CHESSBOARD_FIELDS_COUNT ,
                    path.get(i) % ChessGame.CHESSBOARD_FIELDS_COUNT ,
                    (int) path.get(i) / ChessGame.CHESSBOARD_FIELDS_COUNT
            ) );
        }
