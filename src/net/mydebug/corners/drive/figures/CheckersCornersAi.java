...
//        }
    ...
    private Position getEmptyOnBase() {
        if( color == Figure.BLACK ) {
            ...
            for( int i = ChessGame.CHESSBOARD_FIELDS_COUNT - 1 ; i > ChessGame.CHESSBOARD_FIELDS_COUNT - 5 ; i-- ) {
                for( int j = ChessGame.CHESSBOARD_FIELDS_COUNT - 1 ; j > ChessGame.CHESSBOARD_FIELDS_COUNT - 4 ; j-- ) {
                    Position tmpPosition = new Position( i , j );
                    if( board.checkFieldIsEmpty( tmpPosition ) == 1 ) {
                        return tmpPosition;
                    }
...
//        System.out.println("rebuilded");
        ...
        if( emptyOnBase != null ) {
            ...
            if( emptyOnBase.getX()+1 <  ChessGame.CHESSBOARD_FIELDS_COUNT )
                priorities[ emptyOnBase.getX() + 1 ][ emptyOnBase.getY() ] = 300;
            if( emptyOnBase.getY()+1 <  ChessGame.CHESSBOARD_FIELDS_COUNT )
            ...
            if( nearest.getX()+1 <  ChessGame.CHESSBOARD_FIELDS_COUNT )
                priorities[ nearest.getX() + 1 ][ nearest.getY() ] = 100;
            if( nearest.getY()+1 <  ChessGame.CHESSBOARD_FIELDS_COUNT )
